...
log.title('Mimic V1 Synthesizer')
 ...
 */
synthesizer.init = () => {
  const flitePath = 'bin/mimic-v1'

  /* istanbul ignore if */
  if (process.env.LEON_LANG !== 'en-US') {
    log.warning('The Mimic synthesizer only accepts the "en-US" language for the moment')
  }
...
synthesizer.save = (speech, em, cb) => {
  ...
  const process = spawn('bin/mimic-v1', [
    speech,
